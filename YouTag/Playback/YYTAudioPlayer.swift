...
class YYTAudioPlayer: NSObject, AVAudioPlayerDelegate {
    ...
    */
    ...
    func setupPlayer(withSong songDict: Dictionary<String, Any>) -> Bool {
        ...
        let possibleExtensions = ["m4a", "mp3"]
        var url: URL? = nil
        for ext in possibleExtensions {
            let testURL = LocalFilesManager.getLocalFileURL(withNameAndExtension: "\(songID).\(ext)")
        ...
        guard let finalURL = url else {
            print("❌ 找不到音频文件（m4a 或 mp3）")
            return false
        }
        
        print("📂 试图加载路径: \(finalURL.path)")
        print("📄 文件是否存在: \(FileManager.default.fileExists(atPath: finalURL.path))")
    ...
    if !isSuspended {
        guard let player = audioPlayer else {
            print("❌ audioPlayer is nil, cannot play")
            return
        }
        player.play()
