...
class LibraryViewController: UIViewController, DownloadWebViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        // Update constraint to avoid the notch (Dynamic Island) area
        libraryTableView.topAnchor.constraint(equalTo: self.view.safeAreaLayoutGuide.topAnchor, constant: 10).isActive = true
    ...
    func requestedDownloadLink(link: String, contentType fileExtension: String) {
        let fileURL = URL(fileURLWithPath: link)
        let filename = fileURL.deletingPathExtension().lastPathComponent
        let songTitle = filename.replacingOccurrences(of: "_", with: " ")

        let thumbnailURL = try? URL(
            fileURLWithPath: fileURL.deletingPathExtension().appendingPathExtension("jpg").path
        )

        self.LM.addSongToLibrary(songTitle: songTitle, songUrl: fileURL, songExtension: fileExtension, thumbnailUrl: thumbnailURL, songID: nil, completion: {
            self.libraryTableView.refreshTableView()
