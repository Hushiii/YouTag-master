...
class LibraryManager {
    ...
    */
    func addSongToLibrary(songTitle: String?, songUrl: URL, songExtension: String , thumbnailUrl: URL?, songID: String?, completion: (() -> Void)? = nil) {
        let baseTitle = (songTitle ?? "untitled")
        let safeTitle = baseTitle.components(separatedBy: CharacterSet(charactersIn: "/\\?%*|\"<>")).joined()
        let sID = safeTitle.isEmpty ? "dl_" + generateIDFromTimeStamp() : safeTitle
        ...
        var finalThumbnailURL: URL? = thumbnailUrl
        if finalThumbnailURL == nil, let ytID = songID {
            finalThumbnailURL = URL(string: "https://i.ytimg.com/vi/\(ytID)/hqdefault.jpg")
        }
        
        if let imageUrl = finalThumbnailURL {
            dispatchGroup.enter()
            // Allow spaces in the thumbnail filename, use safeTitle logic
            let thumbnailTitle = safeTitle
            LocalFilesManager.downloadFile(from: imageUrl, filename: thumbnailTitle, extension: "jpg", completion: { error in
                dispatchGroup.leave()
                if error != nil  {
    ...
    func enrichSongDict(_ songDict: Dictionary<String, Any>, fromMetadataDict mdDict: Dictionary<String, Any>) -> Dictionary<String, Any> {
        ...
        for (k, val) in mdDict {
            ...
            } else if key == "artwork" && !LocalFilesManager.checkFileExist(songDict["title"] as! String + ".jpg") {
                if let jpgImageData = UIImage(data: val as! Data)?.jpegData(compressionQuality: 1) {  // make sure image is jpg
                    LocalFilesManager.saveImage(UIImage(data: jpgImageData), withName: songDict["title"] as! String)
                }
    ...
    func importBackupString(bkpString: String) {
        ...
        print(bkpString)
        guard let jsonData = bkpString.data(using: .utf8) else {
            print("❌ Backup string is not valid UTF-8")
            return
        }

        var bkpSongs: [bkpSongStruct] = []
        do {
            bkpSongs = try JSONDecoder().decode([bkpSongStruct].self, from: jsonData)
        } catch {
            print("❌ Failed to parse backup string: \(error.localizedDescription)")
            return
        }
        print(bkpSongs)
